...
import { generateDiagramId, generateId } from '../utils';
import type { DBTable } from '../domain/db-table';
import type { Cardinality, DBRelationship } from '../domain/db-relationship';
import type { DBField } from '../domain/db-field';
import type { DBIndex } from '../domain/db-index';
import type { DataType } from '../domain/db-field';
import { genericDataTypes } from '../data-types';
import { defaultTableColor } from '../colors';
...
export function convertToChartDBDiagram(
    ...
    const tables: DBTable[] = parserResult.tables.map((table, index) => {
        ...
        const fields: DBField[] = table.columns.map((column) => {
            ...
            // Use the mapped type directly (no synonym mapping needed for PostgreSQL)
            const finalType = mappedType;
