...
interface SqlInputSidebarProps {
  ...
  onMergeSQL?: (sql: string, databaseType: DatabaseType) => void;
  onApplySQL?: (sql: string, databaseType: DatabaseType) => void;
...
export function SqlInputSidebar({
  onBackClick,
  onSqlChange,
  onMergeSQL,
  onApplySQL,
  ...
  const saveSuccessTimerRef = React.useRef<ReturnType<
    typeof setTimeout
  > | null>(null);
  ...
  const handleDatabaseTypeChange = (value: DatabaseType) => {
    ...
    if (onSqlChange) onSqlChange(sql, value);
  };
  ...
  const handleClear = () => {
    ...
    if (onSqlChange) onSqlChange("", databaseType);
  };

  const handleApply = React.useCallback(() => {
    if (!sql.trim() || !onApplySQL) return;
    onApplySQL(sql, databaseType);
  }, [sql, databaseType, onApplySQL]);

  const handleMerge = React.useCallback(() => {
    if (!sql.trim() || !onMergeSQL) return;
    onMergeSQL(sql, databaseType);
    setSql("");
  }, [sql, databaseType, onMergeSQL]);
  ...
  const handleFileUpload = React.useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (editMode && onApplySQL) {
          onApplySQL(text, databaseType);
        } else if (onFileLoad) {
    ...
    [databaseType, onSqlChange, onFileLoad, onApplySQL, editMode],
  );
        ...
        <div className="flex items-center justify-between gap-2 px-4 py-3 border-b border-white/10">
          <div className="flex items-center gap-2 flex-1">
            ...
            <h2 className="text-sm font-semibold text-white">
              {editMode ? "Edit Diagram" : "Create New Diagram"}
            </h2>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder={
                  editMode
                    ? "Edit your schema SQL here..."
                    : "Paste your SQL here..."
                }
                className="w-full p-3 border border-white/10 rounded-lg bg-white/5 backdrop-blur-sm text-white text-sm font-mono resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder:text-zinc-500"
