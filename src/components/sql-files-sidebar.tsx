...
export function SqlFilesSidebar({
  className,
  onFileLoad,
  activeFileName,
}: SqlFilesSidebarProps) {
  ...
  const handleShareClick = React.useCallback(
    async (fileId: number, fileName: string, e: React.MouseEvent) => {
      e.stopPropagation();
      try {
        const response = await fetch(`/api/sql-files/${fileId}`);
        if (!response.ok) throw new Error("Failed to fetch file");
        ...
        if (file?.content) {
          ...
          showToast(`"${fileName}" SQL copied to clipboard`, "success");
        }
      } catch {
    ...
    >
      <div className="flex flex-col h-full overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-2xl shadow-[0_20px_70px_-30px_rgba(59,130,246,0.45)] rounded-tl-2xl rounded-tr-2xl">
          ...
          {!collapsed && (
            ...
            >
              ...
              <div className="flex-1 overflow-hidden">
                <h2
                  className="text-sm font-semibold text-white transition-opacity duration-500 ease-out"
                  style={{
                    transitionTimingFunction: "cubic-bezier(0.4, 0, 0.2, 1)",
                  }}
                >
                ...
                <p
                  className="text-xs text-zinc-300 transition-opacity duration-500 ease-out"
                  aria-live="polite"
                  aria-atomic="true"
                  style={{
                    transitionTimingFunction: "cubic-bezier(0.4, 0, 0.2, 1)",
        ...
        <div
          ...
          role="region"
          aria-label="SQL files list"
        >
          {isLoading && (
            <div
              className="flex items-center justify-center py-8"
              aria-live="polite"
              aria-label="Loading SQL files"
            >
              <Loader2
                className="size-5 animate-spin text-zinc-300"
                aria-hidden="true"
              />
          ...
          {error && (
            <div
              className="px-4 py-3 mx-2 my-2 rounded-lg bg-red-900/20 border border-red-800/50"
              role="alert"
              aria-live="assertive"
            >
          ...
          {!isLoading && !error && files.length === 0 && (
            <div className="px-4 py-8 text-center" aria-label="No SQL files">
              <FileText
                className="size-8 mx-auto mb-2 text-zinc-400"
                aria-hidden="true"
              />
              <p className="text-sm text-zinc-300">No SQL files yet</p>
          ...
          {!isLoading && !error && files.length > 0 && (
            <div
              ...
              role="list"
              aria-label={`List of ${files.length} SQL ${files.length === 1 ? "file" : "files"}`}
            >
              {files.map((file) => {
                const isActive =
                  activeFileName &&
                  (activeFileName === file.title ||
                    (!file.title && activeFileName === `File ${file.id}`));
                return (
                  <div
                    key={file.id}
                    className={cn(
                    ...
                    {renamingId === file.id && !collapsed ? (
                      ...
                      <>
                        <button
                          ...
                          disabled={
                            parseMutation.isPending || deletingId === file.id
                          }
                          className={cn(
                          ...
                          aria-describedby={
                            collapsed ? undefined : `file-${file.id}-date`
                          }
                          style={{
                        ...
                        {!collapsed && (
                          <div className="flex items-center gap-0.5 shrink-0">
                            ...
                            <Button
                              ...
                              disabled={deletingId === file.id}
                              className="h-7 w-7 text-zinc-400 hover:text-green-400 hover:bg-green-900/20 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-lg"
                              aria-label={`Share SQL file: ${file.title || `Untitled ${file.id}`}`}
                            >
