 ...
 */
    ...
    function formatDatabaseType(dbType: string | undefined | null): string | null {
        if (!dbType) return null;
        
        // Handle enum values like "sql_server", "mariadb"
        const formatted = dbType
            .replace(/_/g, ' ')
            .split(' ')
    ...
    // Helper function to detect database type from SQL content
    function detectDatabaseType(sqlContent: string): string {
        const upperContent = sqlContent.toUpperCase();
        
        // Check for PostgreSQL dump format
        if (
            sqlContent.includes('SET statement_timeout') ||
            sqlContent.includes('SET lock_timeout') ||
            sqlContent.includes('SET client_encoding') ||
            sqlContent.includes('SELECT pg_catalog.set_config') ||
            sqlContent.includes('ALTER TABLE ONLY') ||
        ...
        if (
            upperContent.includes('SERIAL PRIMARY KEY') ||
            upperContent.includes('CREATE EXTENSION') ||
            upperContent.includes('WITH (OIDS') ||
            upperContent.includes('RETURNS SETOF') ||
            upperContent.includes('DO $$') ||
            upperContent.includes('::TEXT') ||
            upperContent.includes('::INTEGER')
        ) {
            return 'PostgreSQL';
        }
        
        // Check for MySQL/MariaDB-specific keywords
        if (
            upperContent.includes('ENGINE=INNODB') ||
            upperContent.includes('ENGINE=MYISAM') ||
            upperContent.includes('AUTO_INCREMENT') ||
            upperContent.includes('CHARACTER SET') ||
            upperContent.includes('COLLATE') ||
            (upperContent.includes('CREATE TABLE') && upperContent.includes('`'))
        ) {
            // Distinguish between MySQL and MariaDB
            if (upperContent.includes('MARIADB') || upperContent.includes('CONNECT')) {
                return 'MariaDB';
            }
            return 'MySQL';
        }
        ...
        if (
            upperContent.includes('[DBO].') ||
            upperContent.includes('IDENTITY(1,1)') ||
            upperContent.includes('IDENTITY(') ||
            upperContent.includes('NVARCHAR') ||
            upperContent.includes('DATETIME2') ||
            upperContent.includes('UNIQUEIDENTIFIER') ||
            sqlContent.includes('SET ANSI_NULLS ON') ||
            sqlContent.includes('SET QUOTED_IDENTIFIER ON')
        ) {
            return 'SQL Server';
        }
        
        // Check for SQLite-specific keywords
        if (
            upperContent.includes('INTEGER PRIMARY KEY') ||
            upperContent.includes('WITHOUT ROWID') ||
            upperContent.includes('PRAGMA') ||
        ...
        if (
            ...
            upperContent.includes('SYSTIMESTAMP') ||
            upperContent.includes('SYS_GUID') ||
            upperContent.includes('GENERATED ALWAYS AS IDENTITY') ||
            upperContent.includes('GENERATED BY DEFAULT AS IDENTITY') ||
            upperContent.includes('.NEXTVAL') ||
            upperContent.includes('TABLESPACE') ||
            upperContent.includes('CREATE SEQUENCE') ||
            upperContent.includes('CREATE OR REPLACE')
        ) {
            return 'Oracle';
        }
    ...
    const handleSQLContent = React.useCallback((sqlContent: string, fileName: string) => {
        ...
        setDetectedDatabaseType(dbType);
        ...
        <header className="w-full px-4 pt-4">
            ...
            <div className="flex h-16 sm:h-18 lg:h-20 items-center justify-between gap-4">
                ...
                {selectedFileName && !parseMutation.isPending && (
                    ...
                    {(detectedDatabaseType || formatDatabaseType(diagram?.databaseType)) && (
                        <>
