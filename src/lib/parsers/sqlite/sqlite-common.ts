export const parserOpts = { database: 'sqlite' };
...
// Helper to extract table reference (SQLite doesn't support schemas)
export function extractTableReference(table: unknown): string | null {
    ...
    if (table && typeof table === 'object') {
        ...
        if (tableObj.table) {
            return typeof tableObj.table === 'string' ? tableObj.table : extractColumnName(tableObj.table);
        }
